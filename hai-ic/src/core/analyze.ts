/**
 * HAI-IC core analyzer — internal module.
 * HTTP adapters live in app/api/hai-ic/*; integrators use public/ + sdk/.
 */
...
import type { HaiIcResult } from "../public/types";
import { isSincereMode } from "../gate/policy";

export { HAI_IC_PRODUCT, HAI_IC_VERSION };
export type { HaiIcBreakdown, HaiIcResult } from "../public/types";
...
export function analyzeIntent(input: string): HaiIcResult {
  ...
  const risk: string[] = [];
  const sincere = isSincereMode(confidence);
