/**
 * HAI-IC core analyzer — Intent Confidence gate.
 * Boundary: pure scoring + mode decision. No HTTP, no secrets, no Stripe.
 */
...
import type { HaiIcBreakdown, HaiIcGateDecision, HaiIcResult } from "./types";
...
/** Score Intent Confidence and apply Sincere Mode gate (≥75). */
export function analyzeIntent(input: string): HaiIcResult {
  ...
  const breakdown: HaiIcBreakdown = {
    core,
...
/**
 * Pre-LLM gate helper. `allowed` means Sincere Mode passed the 75% gate.
 * Human still owns final execution responsibility.
 */
export function gateIntent(input: string): HaiIcGateDecision {
